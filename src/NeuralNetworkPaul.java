...
public class NeuralNetworkPaul {
	...
	private ArrayList<NeuronPaul> outputNeurons;
	private ArrayList<NeuronPaul> hiddenNeurons;
	...
	public NeuralNetworkPaul(){
		//initialize the neuron lists
		outputNeurons = new ArrayList<NeuronPaul>();
		hiddenNeurons = new ArrayList<NeuronPaul>();
	...
	public void trainNetwork(){
		...
		while(epoch < maxEpochs && totalError > minimumError){
			...
			for(int pattern = 0; pattern < inputs.length; pattern++){
				...
				NeuronPaul n = outputNeurons.get(0); 
				//reset activation
				n.setActivation(0);  
